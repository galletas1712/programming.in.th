...
- At first, I used two different arrays to keep the states, being dp[][][][] and tmp[][][][]. The problem with this approach was that I had to copy data to a fro at every iteration, which gave TLE.
- After moving to the dp[2][][][][] style, the need to copy data was eliminated, since if I set the initial values to INT_MIN, the impossible cases will always turn out to be negative and thus suboptimal. Therefore, I was able to eliminate two 4^4 inner loops, which then passed on all cases.
*/
#include <bits/stdc++.h>
using namespace std;
